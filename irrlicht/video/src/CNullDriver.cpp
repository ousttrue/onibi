...
ITexture* CNullDriver::getTexture(const io::path& filename)
{
	...
	if (file)
	{
		...
		if (texture)
		{
			...
			os::Printer::log(L"Could not load texture", filename, ELL_ERROR);
#else
	...
	{
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"Could not open file of texture", filename, ELL_WARNING);
#else
...
ITexture* CNullDriver::getTexture(io::IReadFile* file)
{
	...
	if (!texture)
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"Could not load texture", file->getFileName(), ELL_WARNING);
#else
...
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"Loaded texture", file->getFileName());
#else
...
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"Could not open file of image", filename, ELL_WARNING);
#else
		...
		s32 userData)
{
	...
	if (vertexShaderProgramFileName.size() )
	{
		...
		{
#if defined(_IRR_IMPROVE_UNICODE)
			os::Printer::log(L"Could not open vertex shader program file",
	...
	if (pixelShaderProgramFileName.size() )
	{
		...
		{
#if defined(_IRR_IMPROVE_UNICODE)
			os::Printer::log(L"Could not open pixel shader program file",
	...
	if (geometryShaderProgramFileName.size() )
	{
		...
		{
#if defined(_IRR_IMPROVE_UNICODE)
			os::Printer::log(L"Could not open geometry shader program file",
