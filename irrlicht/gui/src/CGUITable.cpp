		...
		switch ( Columns[idx].OrderingMode )
		{
			...
			case EGCO_CUSTOM:
				...
				if (Parent)
				{
					...
					event.Info.GUIEvent.EventType = EGET_TABLE_HEADER_CHANGED;
	...
	if (IsEnabled)
	{
		...
		case EET_GUI_EVENT:
			...
			case gui::EGET_SCROLL_BAR_CHANGED:
				if (event.Info.GUIEvent.Caller == VerticalScrollBar)
				{
					// current position will get read out in draw
					return true;
				}
				if (event.Info.GUIEvent.Caller == HorizontalScrollBar)
				{
					// current position will get read out in draw
					return true;
				}
		...
		case EET_MOUSE_INPUT_EVENT:
			{
				...
				case EMIE_MOUSE_WHEEL:
					VerticalScrollBar->setPos(VerticalScrollBar->getPos() + (s32)event.Info.MouseInput.Wheel*-10);
					return true;
				...
				case EMIE_LMOUSE_PRESSED_DOWN:
					...
					if ( dragColumnStart( event.Info.MouseInput.X, event.Info.MouseInput.Y ) )
					{
						Environment->setFocus(this);
						return true;
					}

					if ( selectColumnHeader( event.Info.MouseInput.X, event.Info.MouseInput.Y ) )
						return true;
				...
				case EMIE_MOUSE_MOVED:
					...
					{
						if ( dragColumnUpdate(event.Info.MouseInput.X) )
						{
							return true;
						}
					}
...
void CGUITable::selectNew(s32 ypos, bool onlyHover)
{
	...
	if (Parent && !onlyHover)
	{
		...
		event.Info.GUIEvent.Caller = this;
		event.Info.GUIEvent.Element = 0;
		event.Info.GUIEvent.EventType = (Selected != oldSelected) ? EGET_TABLE_CHANGED : EGET_TABLE_SELECTED_AGAIN;
		Parent->OnEvent(event);
	}
}
