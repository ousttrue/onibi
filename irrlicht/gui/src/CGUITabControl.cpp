	...
	if (IsEnabled)
	{
		...
		case EET_GUI_EVENT:
			...
			case EGET_BUTTON_CLICKED:
				...
				else if (event.Info.GUIEvent.Caller == DownButton)
				{
					scrollRight();
					return true;
				}
		...
		case EET_MOUSE_INPUT_EVENT:
			...
			case EMIE_LMOUSE_LEFT_UP:
				if (selectTab(core::position2d<s32>(event.Info.MouseInput.X, event.Info.MouseInput.Y)))
					return true;
				break;
			default:
				break;
...
bool CGUITabControl::setActiveTab(s32 idx)
{
	...
	if (changed)
	{
		...
		event.Info.GUIEvent.EventType = EGET_TAB_CHANGED;
