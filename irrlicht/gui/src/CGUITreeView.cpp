...
bool CGUITreeView::OnEvent( const SEvent &event )
{
	...
	case EET_GUI_EVENT:
		...
		case gui::EGET_SCROLL_BAR_CHANGED:
			if( event.Info.GUIEvent.Caller == ScrollBarV || event.Info.GUIEvent.Caller == ScrollBarH )
			{
				//s32 pos = ( ( gui::IGUIScrollBar* )event.Info.GUIEvent.Caller )->getPos();
				return true;
			}
			break;
	...
	case EET_MOUSE_INPUT_EVENT:
		{
			...
			case EMIE_MOUSE_WHEEL:
				if ( ScrollBarV )
					ScrollBarV->setPos( ScrollBarV->getPos() + (s32)event.Info.MouseInput.Wheel * -10 );
				return true;
				break;
			...
			case EMIE_LMOUSE_LEFT_UP:
				...
				Selecting = false;
				Environment->removeFocus( this );
				mouseAction( event.Info.MouseInput.X, event.Info.MouseInput.Y );
				return true;
				break;
			...
			case EMIE_MOUSE_MOVED:
				if( Selecting )
				{
					...
					{
						mouseAction( event.Info.MouseInput.X, event.Info.MouseInput.Y, true );
						return true;
					}
				}
				break;
...
void CGUITreeView::mouseAction( s32 xpos, s32 ypos, bool onlyHover /*= false*/ )
{
		...
		&& hitNode->hasChilds() )
	{
		...
		{
			event.Info.GUIEvent.EventType = EGET_TREEVIEW_NODE_EXPAND;
		}
		else
		{
			event.Info.GUIEvent.EventType = EGET_TREEVIEW_NODE_COLLAPS;
		}
	...
	if( Parent && !onlyHover && Selected != oldSelected )
	{
		if( oldSelected )
		{
			event.Info.GUIEvent.EventType = EGET_TREEVIEW_NODE_DESELECT;
		...
		if( Selected )
		{
			event.Info.GUIEvent.EventType = EGET_TREEVIEW_NODE_SELECT;
