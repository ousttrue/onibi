...
bool CGUIWindow::OnEvent(const SEvent& event)
{
	if (IsEnabled)
	{
		...
		case EET_GUI_EVENT:
			...
			if (event.Info.GUIEvent.EventType == EGET_ELEMENT_FOCUSED)
			{
				if (Parent && ((event.Info.GUIEvent.Caller == this) || isMyChild(event.Info.GUIEvent.Caller)))
				{
			...
			if (event.Info.GUIEvent.EventType == EGET_BUTTON_CLICKED)
			{
				if (event.Info.GUIEvent.Caller == CloseButton)
				{
					if (Parent)
					{
						...
						e.Info.GUIEvent.EventType = EGET_ELEMENT_CLOSED;
		...
		case EET_MOUSE_INPUT_EVENT:
			...
			case EMIE_LMOUSE_PRESSED_DOWN:
				DragStart.X = event.Info.MouseInput.X;
				DragStart.Y = event.Info.MouseInput.Y;
			...
			case EMIE_MOUSE_MOVED:
				if (!event.Info.MouseInput.isLeftPressed())
					Dragging = false;
				...
				if (Dragging)
				{
					...
					if (Parent &&
						(event.Info.MouseInput.X < Parent->getAbsolutePosition().UpperLeftCorner.X +1 ||
							event.Info.MouseInput.Y < Parent->getAbsolutePosition().UpperLeftCorner.Y +1 ||
							event.Info.MouseInput.X > Parent->getAbsolutePosition().LowerRightCorner.X -1 ||
							event.Info.MouseInput.Y > Parent->getAbsolutePosition().LowerRightCorner.Y -1))
						return true;

					move(core::position2d<s32>(event.Info.MouseInput.X - DragStart.X, event.Info.MouseInput.Y - DragStart.Y));
					DragStart.X = event.Info.MouseInput.X;
					DragStart.Y = event.Info.MouseInput.Y;
					return true;
				}
