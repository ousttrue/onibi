	...
	if (IsEnabled)
	{
		...
		case EET_MOUSE_INPUT_EVENT:
			...
			case EMIE_MOUSE_WHEEL:
				{
					f32 val = getValue() + (StepSize * event.Info.MouseInput.Wheel);
					setValue(val);
					changeEvent = true;
				}
				break;
		...
		case EET_GUI_EVENT:
			...
			{
				if (event.Info.GUIEvent.Caller == ButtonSpinUp)
				{
					f32 val = getValue();
					val += StepSize;
					setValue(val);
				...
				else if ( event.Info.GUIEvent.Caller == ButtonSpinDown)
				{
					f32 val = getValue();
					val -= StepSize;
					setValue(val);
			...
			if ( event.Info.GUIEvent.EventType == EGET_EDITBOX_CHANGED )
			{
				if (event.Info.GUIEvent.Caller == EditBox)
				{
					verifyValueRange();
					changeEvent = true;
				}
		...
		if ( changeEvent )
		{
			...
			e.Info.GUIEvent.EventType = EGET_SPINBOX_CHANGED;
			if ( Parent )
				Parent->OnEvent(e);
			return true;
		}
