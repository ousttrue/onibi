...
bool CGUIScrollBar::OnEvent(const SEvent& event)
{
	if (IsEnabled)
	{
		...
		case EET_KEY_INPUT_EVENT:
			if (event.Info.KeyInput.PressedDown)
			{
				...
				if (Pos != oldPos)
				{
					...
					newEvent.Info.GUIEvent.EventType = EGET_SCROLL_BAR_CHANGED;
		...
		case EET_GUI_EVENT:
			...
			{
				if (event.Info.GUIEvent.Caller == UpButton)
					setPos(Pos-SmallStep);
				else
				if (event.Info.GUIEvent.Caller == DownButton)
					setPos(Pos+SmallStep);
		...
		{
			const core::position2di p(event.Info.MouseInput.X, event.Info.MouseInput.Y);
			...
			case EMIE_MOUSE_WHEEL:
				if (Environment->hasFocus(this))
				{
					...
					setPos(	getPos() +
							( (s32)event.Info.MouseInput.Wheel * SmallStep * (Horizontal ? 1 : -1 ) )
							);
			...
			case EMIE_MOUSE_MOVED:
			{
				...
				if (!DraggedBySlider)
				{
					...
					{
						TrayClick = false;
						if (event.Info.MouseInput.Event == EMIE_MOUSE_MOVED)
							return isInside;
					}
				}
