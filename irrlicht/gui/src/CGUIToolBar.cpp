...
bool CGUIToolBar::OnEvent(const SEvent& event)
{
	if (IsEnabled)
	{
		if (event.EventType == EET_MOUSE_INPUT_EVENT &&
			event.Info.MouseInput.Event == EMIE_LMOUSE_PRESSED_DOWN)
		{
			if (AbsoluteClippingRect.isPointInside(core::position2di(event.Info.MouseInput.X, event.Info.MouseInput.Y)))
				return true;
		}
	}
