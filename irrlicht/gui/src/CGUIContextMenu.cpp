	...
	if (IsEnabled)
	{
		...
		case EET_GUI_EVENT:
			...
			case EGET_ELEMENT_FOCUS_LOST:
				if (event.Info.GUIEvent.Caller == this && !isMyChild(event.Info.GUIEvent.Element) && AllowFocus)
				{
			...
			case EGET_ELEMENT_FOCUSED:
				if (event.Info.GUIEvent.Caller == this && !AllowFocus)
				{
					return true;
				}
				break;
		...
		case EET_MOUSE_INPUT_EVENT:
			...
			case EMIE_LMOUSE_LEFT_UP:
				{
					...
					grab();
					const u32 t = sendClick(core::position2d<s32>(event.Info.MouseInput.X, event.Info.MouseInput.Y));
			...
			case EMIE_MOUSE_MOVED:
				if (Environment->hasFocus(this))
					highlight(core::position2d<s32>(event.Info.MouseInput.X, event.Info.MouseInput.Y), true);
				return true;
			default:
				break;
			}
...
u32 CGUIContextMenu::sendClick(const core::position2d<s32>& p)
{
		...
		(u32)HighLighted < Items.size())
	{
		...
		event.Info.GUIEvent.EventType = EGET_MENU_ITEM_SELECTED;
		if (EventParent)
