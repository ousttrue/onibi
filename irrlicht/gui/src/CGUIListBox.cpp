	...
	if (IsEnabled)
	{
		...
		case EET_KEY_INPUT_EVENT:
			if (event.Info.KeyInput.PressedDown &&
				(event.Info.KeyInput.Key == KEY_DOWN ||
				event.Info.KeyInput.Key == KEY_UP   ||
				event.Info.KeyInput.Key == KEY_HOME ||
				event.Info.KeyInput.Key == KEY_END  ||
				event.Info.KeyInput.Key == KEY_NEXT ||
				event.Info.KeyInput.Key == KEY_PRIOR ) )
			{
				...
				if (oldSelected != Selected && Parent && !Selecting && !MoveOverSelect)
				{
					...
					e.Info.GUIEvent.Caller = this;
					e.Info.GUIEvent.Element = 0;
					e.Info.GUIEvent.EventType = EGET_LISTBOX_CHANGED;
					Parent->OnEvent(e);
				}
			...
			if (!event.Info.KeyInput.PressedDown && ( event.Info.KeyInput.Key == KEY_RETURN || event.Info.KeyInput.Key == KEY_SPACE ) )
			{
				if (Parent)
				{
					SEvent e;
					...
					e.Info.GUIEvent.Caller = this;
					e.Info.GUIEvent.Element = 0;
					e.Info.GUIEvent.EventType = EGET_LISTBOX_SELECTED_AGAIN;
					Parent->OnEvent(e);
				}
				return true;
			}
			else if (event.Info.KeyInput.PressedDown && event.Info.KeyInput.Char)
			{
				...
				if (now - LastKeyTime < 500)
				{
					...
					if (!(KeyBuffer.size() == 1 && KeyBuffer[0] == event.Info.KeyInput.Char))
					{
						KeyBuffer += L" ";
						KeyBuffer[KeyBuffer.size()-1] = event.Info.KeyInput.Char;
					}
				}
				else
				{
					KeyBuffer = L" ";
					KeyBuffer[0] = event.Info.KeyInput.Char;
				}
				LastKeyTime = now;
		...
		case EET_GUI_EVENT:
			...
			case gui::EGET_SCROLL_BAR_CHANGED:
				if (event.Info.GUIEvent.Caller == ScrollBar)
					return true;
				break;
			case gui::EGET_ELEMENT_FOCUS_LOST:
				{
					if (event.Info.GUIEvent.Caller == this)
						Selecting = false;
				}
			default:
			break;
		...
		case EET_MOUSE_INPUT_EVENT:
			{
				core::position2d<s32> p(event.Info.MouseInput.X, event.Info.MouseInput.Y);

				switch(event.Info.MouseInput.Event)
				{
				case EMIE_MOUSE_WHEEL:
					ScrollBar->setPos(ScrollBar->getPos() + (s32)event.Info.MouseInput.Wheel*-ItemHeight/2);
					return true;
				...
				case EMIE_LMOUSE_LEFT_UP:
				{
					...
					if (isPointInside(p))
						selectNew(event.Info.MouseInput.Y);
				...
				case EMIE_MOUSE_MOVED:
					if (Selecting || MoveOverSelect)
					{
						if (isPointInside(p))
						{
							selectNew(event.Info.MouseInput.Y, true);
							return true;
						}
					}
				default:
...
void CGUIListBox::selectNew(s32 ypos, bool onlyHover)
{
	...
	if (Parent && !onlyHover)
	{
		...
		event.Info.GUIEvent.Caller = this;
		event.Info.GUIEvent.Element = 0;
		event.Info.GUIEvent.EventType = eventType;
