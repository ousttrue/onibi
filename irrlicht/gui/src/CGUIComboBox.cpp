...
bool CGUIComboBox::OnEvent(const SEvent& event)
{
	if (IsEnabled)
	{
		...
		case EET_KEY_INPUT_EVENT:
			if (ListBox && event.Info.KeyInput.PressedDown && event.Info.KeyInput.Key == KEY_ESCAPE)
			{
				// hide list box
				openCloseMenu();
				return true;
			...
			if (event.Info.KeyInput.Key == KEY_RETURN || event.Info.KeyInput.Key == KEY_SPACE)
			{
		...
		case EET_GUI_EVENT:
			...
			case EGET_ELEMENT_FOCUS_LOST:
				if (ListBox &&
					(Environment->hasFocus(ListBox) || ListBox->isMyChild(event.Info.GUIEvent.Caller) ) &&
					event.Info.GUIEvent.Element != this &&
					!isMyChild(event.Info.GUIEvent.Element) &&
					!ListBox->isMyChild(event.Info.GUIEvent.Element))
				{
					openCloseMenu();
				}
				break;
			case EGET_BUTTON_CLICKED:
				if (event.Info.GUIEvent.Caller == ListButton)
				{
					openCloseMenu();
					return true;
				}
			...
			case EGET_LISTBOX_CHANGED:
				if (event.Info.GUIEvent.Caller == ListBox)
				{
		...
		case EET_MOUSE_INPUT_EVENT:
			...
			case EMIE_MOUSE_WHEEL:
				{
					s32 oldSelected = Selected;
					setSelected( Selected + ((event.Info.MouseInput.Wheel < 0) ? 1 : -1));
...
void CGUIComboBox::sendSelectionChangedEvent()
{
	if (Parent)
	{
		...
		event.Info.GUIEvent.EventType = EGET_COMBO_BOX_CHANGED;
