...
bool CGUIEditBox::processKey(const SEvent& event)
{
	...
	if (event.Info.KeyInput.Control)
	{
		// german backlash '\' entered with control + '?'
		if ( event.Info.KeyInput.Char == '\\' )
		{
			inputChar(event.Info.KeyInput.Char);
			return true;
		}
		...
		case KEY_DOWN:
			if (MultiLine || (WordWrap && BrokenText.size() > 1) )
			{
				if(UTF16_IS_SURROGATE_LO(Text[CursorPos]))
				{
					SEvent leftEvent;
					leftEvent = event;
					leftEvent.Info.KeyInput.Key = KEY_LEFT;
...
{
	switch(event.Info.InputMethodEvent.Event)
	{
		case EIME_CHAR_INPUT:
			inputChar(event.Info.InputMethodEvent.Char);
			return true;
		case EIME_CHANGE_POS:
			{
				...
				irr::updateICPos(event.Info.InputMethodEvent.Handle, pos.X,pos.Y, font->getDimension(L"|").Height);
...
bool CGUIEditBox::processMouse(const SEvent& event)
{
	...
	case irr::EMIE_LMOUSE_LEFT_UP:
		...
		{
			CursorPos = getCursorPos(event.Info.MouseInput.X, event.Info.MouseInput.Y);
#if defined(_IRR_IMPROVE_UNICODE)
			if(UTF16_IS_SURROGATE_LO(Text[CursorPos]))
			{
				if(CursorPos>0)
	...
	case EMIE_LMOUSE_PRESSED_DOWN:
		...
		else
		{
			if (!AbsoluteClippingRect.isPointInside(
				core::position2d<s32>(event.Info.MouseInput.X, event.Info.MouseInput.Y)))
			{
				return false;
			}
			else
...
void CGUIEditBox::sendGuiEvent(EGUI_EVENT_TYPE type)
{
	if ( Parent )
	{
        ...
        e.Info.GUIEvent.EventType = type;
