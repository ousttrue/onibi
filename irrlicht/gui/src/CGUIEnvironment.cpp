...
bool CGUIEnvironment::setFocus(IGUIElement* element)
{
	...
	if (Focus)
	{
		...
		e.Info.GUIEvent.Caller = Focus;
		e.Info.GUIEvent.Element = element;
		e.Info.GUIEvent.EventType = EGET_ELEMENT_FOCUS_LOST;
		if (Focus->OnEvent(e))
		{
			if (element)
				element->drop();
	...
	if (element)
	{
		...
		SEvent e;
		e.EventType = EET_GUI_EVENT;
		e.Info.GUIEvent.Caller = element;
		e.Info.GUIEvent.Element = Focus;
		e.Info.GUIEvent.EventType = EGET_ELEMENT_FOCUSED;
...
bool CGUIEnvironment::OnEvent(const SEvent& event)
{
	...
	if (UserReceiver
		...
		&& (event.EventType != EET_GUI_EVENT || event.Info.GUIEvent.Caller != this))
	{
...
void CGUIEnvironment::updateHoveredElement(core::position2d<s32> mousePos)
{
	...
	if (Hovered)
	{
		...
		if (Hovered != lastHovered)
		{
			...
			if (lastHovered)
			{
				event.Info.GUIEvent.Caller = lastHovered;
				event.Info.GUIEvent.EventType = EGET_ELEMENT_LEFT;
			...
			event.Info.GUIEvent.Caller  = Hovered;
			event.Info.GUIEvent.Element = Hovered;
			event.Info.GUIEvent.EventType = EGET_ELEMENT_HOVERED;
...
bool CGUIEnvironment::postEventFromUser(const SEvent& event)
{
	...
	case EET_MOUSE_INPUT_EVENT:
		...
		updateHoveredElement(core::position2d<s32>(event.Info.MouseInput.X, event.Info.MouseInput.Y));
	...
	case EET_KEY_INPUT_EVENT:
		{
			...
			if (event.EventType == EET_KEY_INPUT_EVENT &&
				...
				event.Info.KeyInput.Key == KEY_TAB)
			{
				IGUIElement *next = getNextElement(event.Info.KeyInput.Shift, event.Info.KeyInput.Control);
				if (next && next != Focus)
				{
					if (setFocus(next))
						return true;
...
bool CGUIEnvironment::loadGUI(const io::path& filename, IGUIElement* parent)
{
	...
	if (!read)
	{
		...
		os::Printer::log(L"Unable to open gui file", filename, ELL_ERROR);
#else
...
bool CGUIEnvironment::loadGUI(io::IReadFile* file, IGUIElement* parent)
{
	...
	{
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"GUI is not a valid XML file", file->getFileName(), ELL_ERROR);
#else
...
IGUIFont* CGUIEnvironment::getFont(const io::path& filename)
{
	...
	{
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"Could not load font because the file does not exist", f.NamedPath.getPath(), ELL_ERROR);
#else
	...
	if (xml)
	{
		...
		else if (t==EGFT_VECTOR)
		{
			// todo: vector fonts
#if defined(_IRR_IMPROVE_UNICODE)
			os::Printer::log(L"Unable to load font, XML vector fonts are not supported yet", f.NamedPath, ELL_ERROR);
#else
...
#if defined(_IRR_COMPILE_WITH_CGUITTFONT_)
//! returns the font
IGUIFont* CGUIEnvironment::getFont(const io::path& filename, u32 fontsize, bool antialias = true, bool transparency = true)
{
	// search existing font
	...
	{
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"Could not load font because the file does not exist", tf.NamedPath.getPath(), ELL_ERROR);
#else
	...
	CGUITTFont* font = CGUITTFont::createTTFont(this, tf.NamedPath.getPath(), tf.size);
...
IGUISpriteBank* CGUIEnvironment::getSpriteBank(const io::path& filename)
{
	...
	{
#if defined(_IRR_IMPROVE_UNICODE)
		os::Printer::log(L"Could not load sprite bank because the file does not exist", b.NamedPath.getPath(), ELL_ERROR);
#else
