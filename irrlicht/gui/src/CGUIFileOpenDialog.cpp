		...
		case EET_GUI_EVENT:
			...
			case EGET_BUTTON_CLICKED:
				if (event.Info.GUIEvent.Caller == CloseButton ||
					event.Info.GUIEvent.Caller == CancelButton)
				{
					sendCancelEvent();
					remove();
					return true;
				...
				if (event.Info.GUIEvent.Caller == OKButton )
				{
					if ( FileDirectory != L"" )
					{
			...
			case EGET_EDITBOX_ENTER:
				if (event.Info.GUIEvent.Caller == FileNameText)
				{
		...
		case EET_MOUSE_INPUT_EVENT:
			...
			case EMIE_MOUSE_MOVED:
				...
				if (Dragging)
				{
					...
					if (Parent)
						if (event.Info.MouseInput.X < Parent->getAbsolutePosition().UpperLeftCorner.X +1 ||
...
void CGUIFileOpenDialog::sendSelectedEvent( EGUI_EVENT_TYPE type)
{
	...
	event.Info.GUIEvent.EventType = type;
...
void CGUIFileOpenDialog::sendCancelEvent()
{
	...
	event.Info.GUIEvent.Caller = this;
	event.Info.GUIEvent.Element = 0;
	event.Info.GUIEvent.EventType = EGET_FILE_CHOOSE_DIALOG_CANCELLED;
	Parent->OnEvent(event);
}
