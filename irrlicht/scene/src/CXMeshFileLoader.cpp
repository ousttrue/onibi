...
bool CXMeshFileLoader::load(io::IReadFile* file)
{
	...
	for (u32 n=0; n<Meshes.size(); ++n)
	{
		...
		#else
		{
			...
			for (u32 j=0; j<mesh->WeightJoint.size(); ++j)
			{
				SWeight& weight = (AnimatedMesh->getAllJoints()[mesh->WeightJoint[j]]->Weights[mesh->WeightNum[j]]);
...
bool CXMeshFileLoader::parseDataObjectFrame(SJoint *Parent)
{
#ifdef _XREADER_DEBUG
...
bool CXMeshFileLoader::parseDataObjectSkinWeights(SXMesh &mesh)
{
	...
	for (i=0; i<nWeights; ++i)
	{
		...
		SWeight *weight=AnimatedMesh->addWeight(joint);
...
bool CXMeshFileLoader::parseDataObjectAnimationKey(SJoint *joint)
{
#ifdef _XREADER_DEBUG
		...
		case 0: //rotation
			{
				...
				SRotationKey *key=AnimatedMesh->addRotationKey(joint);
				key->frame=time;
		...
		case 2: //position
			{
				...
				if (keyType==2)
				{
					SPositionKey *key=AnimatedMesh->addPositionKey(joint);
					key->frame=time;
					key->position=vector;
				}
				else
				{
					SScaleKey *key=AnimatedMesh->addScaleKey(joint);
					key->frame=time;
					key->scale=vector;
				}
			}
		...
		case 4:
			{
				...
				SRotationKey *keyR=AnimatedMesh->addRotationKey(joint);
				keyR->frame=time;
				keyR->rotation= core::quaternion(mat);


				SPositionKey *keyP=AnimatedMesh->addPositionKey(joint);
				keyP->frame=time;
