...
//	frameCount = os::Byteswap::byteswap(frameCount);
	...
	for (i=0; i<jointCount; ++i)
	{
		...
		SJoint *jnt = AnimatedMesh->addJoint();
		...
		for (j=0; j < numRotationKeyframes; ++j)
		{
			...
			SRotationKey *k=AnimatedMesh->addRotationKey(jnt);
		...
		for (j=0; j<numTranslationKeyframes; ++j)
		{
			...
			SPositionKey *k=AnimatedMesh->addPositionKey(jnt);
			k->frame = kf->Time * framesPerSecond-1;
		...
		for (s32 j = 2; j!=-1; --j)
		{
			...
			if (index == -1)
			{
				...
				if (vertexWeights.size()==0)
				{
					...
					{
						SWeight *w=AnimatedMesh->addWeight(AnimatedMesh->getAllJoints()[boneid]);
						w->buffer_id = matidx;
						w->strength = 1.0f;
						w->vertex_id = index;
					}
