...
bool CB3DMeshFileLoader::readChunkNODE(SJoint *inJoint)
{
	SJoint *joint = AnimatedMesh->addJoint(inJoint);
...
bool CB3DMeshFileLoader::readChunkMESH(SJoint *inJoint)
{
#ifdef _B3D_READER_DEBUG
...
*/
bool CB3DMeshFileLoader::readChunkVRTS(SJoint *inJoint)
{
#ifdef _B3D_READER_DEBUG
...
bool CB3DMeshFileLoader::readChunkBONE(SJoint *inJoint)
{
#ifdef _B3D_READER_DEBUG
	...
	if (B3dStack.getLast().length > 8)
	{
		while((B3dStack.getLast().startposition + B3dStack.getLast().length) > B3DFile->getPos()) // this chunk repeats
		{
			...
			{
				SWeight *weight=AnimatedMesh->addWeight(inJoint);
...
bool CB3DMeshFileLoader::readChunkKEYS(SJoint *inJoint)
{
#ifdef _B3D_READER_DEBUG
//	os::Printer::log("read ChunkKEYS");
	...
	SPositionKey *oldPosKey=0;
	core::vector3df oldPos[2];
	SScaleKey *oldScaleKey=0;
	core::vector3df oldScale[2];
	SRotationKey *oldRotKey=0;
