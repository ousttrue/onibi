...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if ( m->group >= 0 )
	{
		...
		event.Info.MouseInput.Event = (irr::EMOUSE_INPUT_EVENT) m->irrMessage;
		...
		if (wParam & MK_XBUTTON1)
			event.Info.MouseInput.ButtonStates |= irr::EMBSM_EXTRA1;
		if (wParam & MK_XBUTTON2)
			event.Info.MouseInput.ButtonStates |= irr::EMBSM_EXTRA2;
#endif
		event.Info.MouseInput.Wheel = 0.f;
		...
		if ( m->group == 3 )
		{
			...
			event.Info.MouseInput.X -= p.x;
			event.Info.MouseInput.Y -= p.y;
		...
		if (dev)
		{
			...
			if ( event.Info.MouseInput.Event >= irr::EMIE_LMOUSE_PRESSED_DOWN && event.Info.MouseInput.Event <= irr::EMIE_MMOUSE_PRESSED_DOWN )
			{
				irr::u32 clicks = dev->checkSuccessiveClicks(event.Info.MouseInput.X, event.Info.MouseInput.Y, event.Info.MouseInput.Event);
				if ( clicks == 2 )
				{
					event.Info.MouseInput.Event = (irr::EMOUSE_INPUT_EVENT)(irr::EMIE_LMOUSE_DOUBLE_CLICK + event.Info.MouseInput.Event-irr::EMIE_LMOUSE_PRESSED_DOWN);
					dev->postEventFromUser(event);
				}
				else if ( clicks == 3 )
				{
					event.Info.MouseInput.Event = (irr::EMOUSE_INPUT_EVENT)(irr::EMIE_LMOUSE_TRIPLE_CLICK + event.Info.MouseInput.Event-irr::EMIE_LMOUSE_PRESSED_DOWN);
					dev->postEventFromUser(event);
				}
			}
		}
		...
		if(m->irrMessage == irr::EMIE_LMOUSE_PRESSED_DOWN || m->irrMessage == irr::EMIE_LMOUSE_LEFT_UP)		
		{
			...
			event.Info.InputMethodEvent.Event = irr::EIME_CHANGE_POS;
			event.Info.InputMethodEvent.Handle = hWnd;
	...
	case WM_IME_CHAR:
		{
			event.EventType = irr::EET_IMPUT_METHOD_EVENT;
			event.Info.InputMethodEvent.Event = irr::EIME_CHAR_INPUT;
			event.Info.InputMethodEvent.Handle = hWnd;
			event.Info.InputMethodEvent.Char = (wchar_t)wParam;
	...
	case WM_KEYUP:
		{
			...
			if ( event.Info.KeyInput.Key == irr::KEY_SHIFT )
			{
				// this will fail on systems before windows NT/2000/XP, not sure _what_ will return there instead.
				event.Info.KeyInput.Key = (irr::EKEY_CODE)MapVirtualKey( ((lParam>>16) & 255), MY_MAPVK_VSC_TO_VK_EX );
			}
			if ( event.Info.KeyInput.Key == irr::KEY_CONTROL )
			{
				event.Info.KeyInput.Key = (irr::EKEY_CODE)MapVirtualKey( ((lParam>>16) & 255), MY_MAPVK_VSC_TO_VK_EX );
				// some keyboards will just return LEFT for both - left and right keys. So also check extend bit.
				if (lParam & 0x1000000)
					event.Info.KeyInput.Key = irr::KEY_RCONTROL;
			}
			if ( event.Info.KeyInput.Key == irr::KEY_MENU )
			{
				event.Info.KeyInput.Key = (irr::EKEY_CODE)MapVirtualKey( ((lParam>>16) & 255), MY_MAPVK_VSC_TO_VK_EX );
				if (lParam & 0x1000000)
					event.Info.KeyInput.Key = irr::KEY_RMENU;
			}
			...
			if (conversionResult == 1)
			{
				...
				MultiByteToWideChar(
						...
						(WCHAR*)&unicodeChar,
						1 );
				event.Info.KeyInput.Char = unicodeChar; 
			}
			else
				event.Info.KeyInput.Char = 0;

			// allow composing characters like '@' with Alt Gr on non-US keyboards
			if ((allKeys[VK_MENU] & 0x80) != 0)
				event.Info.KeyInput.Control = 0;
	...
	case WM_USER:
		...
		event.Info.UserEvent.UserData1 = (irr::s32)wParam;
		event.Info.UserEvent.UserData2 = (irr::s32)lParam;
		dev = getDeviceFromHWnd(hWnd);
	...
	for(; joystick < numberOfJoysticks; ++joystick)
	{
											...
											sizeof(activeJoystick.Caps)))
		{
			...
			returnInfo.PovHat = ((activeJoystick.Caps.wCaps & JOYCAPS_HASPOV) == JOYCAPS_HASPOV)
								? POV_HAT_PRESENT : POV_HAT_ABSENT;
...
void CIrrDeviceWin32::pollJoysticks()
{
	...
	for(joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
	{
		...
		if(JOYERR_NOERROR == joyGetPosEx(ActiveJoysticks[joystick].Index, &info))
		{
			...
			event.Info.JoystickEvent.Joystick = (u8)joystick;
 
			event.Info.JoystickEvent.POV = (u16)info.dwPOV;
			// set to undefined if no POV value was returned or the value
			// is out of range
			if (!(info.dwFlags & JOY_RETURNPOV) || (event.Info.JoystickEvent.POV > 35900))
				event.Info.JoystickEvent.POV = 65535;

			for(int axis = 0; axis < NUMBER_OF_AXES; ++axis)
				event.Info.JoystickEvent.Axis[axis] = 0;

			switch(caps.wNumAxes)
			{
			default:
			case 6:
				event.Info.JoystickEvent.Axis[AXIS_V] =
					(s16)((65535 * (info.dwVpos - caps.wVmin)) / (caps.wVmax - caps.wVmin) - 32768);

			case 5:
				event.Info.JoystickEvent.Axis[AXIS_U] =
					(s16)((65535 * (info.dwUpos - caps.wUmin)) / (caps.wUmax - caps.wUmin) - 32768);

			case 4:
				event.Info.JoystickEvent.Axis[AXIS_R] =
					(s16)((65535 * (info.dwRpos - caps.wRmin)) / (caps.wRmax - caps.wRmin) - 32768);

			case 3:
				event.Info.JoystickEvent.Axis[AXIS_Z] =
					(s16)((65535 * (info.dwZpos - caps.wZmin)) / (caps.wZmax - caps.wZmin) - 32768);

			case 2:
				event.Info.JoystickEvent.Axis[AXIS_Y] =
					(s16)((65535 * (info.dwYpos - caps.wYmin)) / (caps.wYmax - caps.wYmin) - 32768);

			case 1:
				event.Info.JoystickEvent.Axis[AXIS_X] =
					(s16)((65535 * (info.dwXpos - caps.wXmin)) / (caps.wXmax - caps.wXmin) - 32768);
			}

			event.Info.JoystickEvent.ButtonStates = info.dwButtons;
