...
struct JoystickInfo
{
	...
	JoystickInfo() : hats(0), axes(0), buttons(0), interface(0), removed(false), usage(0), usagePage(0), numActiveJoysticks(0)
	{
		...
		persistentData.Info.JoystickEvent.POV = 65535;
		persistentData.Info.JoystickEvent.ButtonStates = 0;
	}
};
	...
	if (event != nil)
	{
		...
		switch([(NSEvent *)event type])
		{
			...
			case NSFlagsChanged:
				...
				ievent.Info.KeyInput.Shift = ([(NSEvent *)event modifierFlags] & NSShiftKeyMask) != 0;
				ievent.Info.KeyInput.Control = ([(NSEvent *)event modifierFlags] & NSControlKeyMask) != 0;

				if (IsShiftDown != ievent.Info.KeyInput.Shift)
				{
					ievent.Info.KeyInput.Char = irr::KEY_SHIFT;
					ievent.Info.KeyInput.Key = irr::KEY_SHIFT;
					ievent.Info.KeyInput.PressedDown = ievent.Info.KeyInput.Shift;

					IsShiftDown = ievent.Info.KeyInput.Shift;

					postEventFromUser(ievent);
				}

				if (IsControlDown != ievent.Info.KeyInput.Control)
				{
					ievent.Info.KeyInput.Char = irr::KEY_CONTROL;
					ievent.Info.KeyInput.Key = irr::KEY_CONTROL;
					ievent.Info.KeyInput.PressedDown = ievent.Info.KeyInput.Control;

					IsControlDown = ievent.Info.KeyInput.Control;
			...
			case NSLeftMouseDown:
				...
				ievent.Info.MouseInput.Event = irr::EMIE_LMOUSE_PRESSED_DOWN;
				MouseButtonStates |= irr::EMBSM_LEFT;
				ievent.Info.MouseInput.ButtonStates = MouseButtonStates;
			...
			case NSLeftMouseUp:
				...
				MouseButtonStates &= !irr::EMBSM_LEFT;
				ievent.Info.MouseInput.ButtonStates = MouseButtonStates;
				ievent.Info.MouseInput.Event = irr::EMIE_LMOUSE_LEFT_UP;
				postMouseEvent(event,ievent);
				break;

			case NSOtherMouseDown:
				ievent.EventType = irr::EET_MOUSE_INPUT_EVENT;
				ievent.Info.MouseInput.Event = irr::EMIE_MMOUSE_PRESSED_DOWN;
			...
			case NSOtherMouseUp:
				...
				MouseButtonStates &= !irr::EMBSM_MIDDLE;
				ievent.Info.MouseInput.ButtonStates = MouseButtonStates;
				ievent.Info.MouseInput.Event = irr::EMIE_MMOUSE_LEFT_UP;
				postMouseEvent(event,ievent);
				break;
			...
			case NSOtherMouseDragged:
				ievent.EventType = irr::EET_MOUSE_INPUT_EVENT;
				ievent.Info.MouseInput.Event = irr::EMIE_MOUSE_MOVED;
				ievent.Info.MouseInput.ButtonStates = MouseButtonStates;
				postMouseEvent(event,ievent);
				break;
			...
			case NSRightMouseDown:
				ievent.EventType = irr::EET_MOUSE_INPUT_EVENT;
				ievent.Info.MouseInput.Event = irr::EMIE_RMOUSE_PRESSED_DOWN;
			...
			case NSRightMouseUp:
				ievent.EventType = irr::EET_MOUSE_INPUT_EVENT;
				ievent.Info.MouseInput.Event = irr::EMIE_RMOUSE_LEFT_UP;
				MouseButtonStates &= !irr::EMBSM_RIGHT;
				ievent.Info.MouseInput.ButtonStates = MouseButtonStates;
				postMouseEvent(event,ievent);
				break;
			...
			case NSScrollWheel:
				ievent.EventType = irr::EET_MOUSE_INPUT_EVENT;
				ievent.Info.MouseInput.Event = irr::EMIE_MOUSE_WHEEL;
				ievent.Info.MouseInput.Wheel = [(NSEvent *)event deltaY];
				if (ievent.Info.MouseInput.Wheel < 1.0f)
					ievent.Info.MouseInput.Wheel *= 10.0f;
				else
					ievent.Info.MouseInput.Wheel *= 5.0f;
				postMouseEvent(event,ievent);
				break;
...
void CIrrDeviceMacOSX::postKeyEvent(void *event,irr::SEvent &ievent,bool pressed)
{
	...
	if ((str != nil) && ([str length] > 0))
	{
		...
		ievent.EventType = irr::EET_KEY_INPUT_EVENT;
		ievent.Info.KeyInput.Key = (irr::EKEY_CODE)mkey;
		ievent.Info.KeyInput.PressedDown = pressed;
		ievent.Info.KeyInput.Shift = ([(NSEvent *)event modifierFlags] & NSShiftKeyMask) != 0;
		ievent.Info.KeyInput.Control = ([(NSEvent *)event modifierFlags] & NSControlKeyMask) != 0;
		ievent.Info.KeyInput.Char = mchar;

		if (skipCommand)
			ievent.Info.KeyInput.Control = true;
...
void CIrrDeviceMacOSX::postMouseEvent(void *event,irr::SEvent &ievent)
{
	...
	if (Window != NULL)
	{
		ievent.Info.MouseInput.X = (int)[(NSEvent *)event locationInWindow].x;
		ievent.Info.MouseInput.Y = DeviceHeight - (int)[(NSEvent *)event locationInWindow].y;

		if (ievent.Info.MouseInput.Y < 0)
			post = false;
	}
	else
	{
		ievent.Info.MouseInput.X = (int)[NSEvent mouseLocation].x;
		ievent.Info.MouseInput.Y = DeviceHeight - (int)[NSEvent mouseLocation].y;
	}
	...
	while ((hidObject = IOIteratorNext (hidIterator)))
	{
		...
		if ((kern_result == KERN_SUCCESS) && hidProperties)
		{
			...
			if (info.interface != NULL)
			{
				...
				for (u32 i = 0; i < 6; ++i)
					info.persistentData.Info.JoystickEvent.Axis[i] = 0;
...
void CIrrDeviceMacOSX::pollJoysticks()
{
	...
	for (joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
	{
		...
		bool found = false;
		ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.Joystick = joystick;
		...
		if (ActiveJoysticks[joystick].interface)
		{
			for (u32 n = 0; n < ActiveJoysticks[joystick].axisComp.size(); n++)
			{
				...
				if (kIOReturnSuccess == result)
				{
					...
					if (ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.Axis[n] != (s16)hidEvent.value)
						found = true;
					ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.Axis[n] = (s16)hidEvent.value;
				}
			}//axis check
			...
			for (u32 n = 0; n < ActiveJoysticks[joystick].buttonComp.size(); n++)
			{
				...
				if (kIOReturnSuccess == result)
				{
					...
					if (hidEvent.value && !((ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.ButtonStates & (1 << n)) ? true : false) )
							found = true;
					else if (!hidEvent.value && ((ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.ButtonStates & (1 << n)) ? true : false))
							found = true;

					if (hidEvent.value)
							ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.ButtonStates |= (1 << n);
					else
							ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.ButtonStates &= ~(1 << n);
				}
			}//button check
...
/*
			for (u32 n = 0; n < ActiveJoysticks[joystick].hatComp.size(); n++)
			{
				...
				if (kIOReturnSuccess == result)
				{
					if (ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.POV != hidEvent.value)
						found = true;
					ActiveJoysticks[joystick].persistentData.Info.JoystickEvent.POV = hidEvent.value;
				}
			}//hat check
*/
		}
