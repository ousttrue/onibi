  ...
  namespace pmd {
    ...
    template<class READER>
      ...
      read(READER &reader, Vertex &v)
      {
        ...
        assert(reader.getPos()-base_pos==38);
      }
    ...
    template<class READER>
      ...
      read(READER &reader, Material &m)
      {
        ...
        assert(reader.getPos()-base_pos==70);
      }
    ...
    template<class READER>
      ...
      read(READER &reader, Bone &b)
      {
        ...
        reader.get(b.pos);
        assert(reader.getPos()-base_pos==39);
      }
    ...
    template<class READER>
      ...
      read(READER &reader, RigidBody &r)
      {
        ...
        assert(reader.getPos()-base_pos==83);
      }
    ...
    bool IO::read(binary::IReader &input)
    {
      ...
      if(!morph_list.empty()){
        ...
        for(size_t i=1; i<morph_list.size(); ++i){
          assert(morph_list[i].type!=MORPH_BASE);
        }
      }
      ////////////////////////////////////////////////////////////
      // setup bone
