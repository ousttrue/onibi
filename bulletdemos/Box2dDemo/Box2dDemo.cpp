...
//		float start_z = START_POS_Z - ARRAY_SIZE_Z/2;
		...
		for (int i = 0; i < ARRAY_SIZE_X; ++i)
		{
			...
			for (int  j = i; j < ARRAY_SIZE_Y; ++j)
			{
					...
					btRigidBodyConstructionInfo rbInfo(0,0,0);
					switch (j%3)
					{
#if 1
					case 0:
						rbInfo = btRigidBodyConstructionInfo(mass,myMotionState,colShape,localInertia);
						break;
					case 1:
						rbInfo = btRigidBodyConstructionInfo(mass,myMotionState,colShape3,localInertia);
						break;
#endif
					default:
						rbInfo = btRigidBodyConstructionInfo(mass,myMotionState,colShape2,localInertia);
					}
					btRigidBody* body = new btRigidBody(rbInfo);
