...
void Init_Floatyness (HfFluidDemo* fluidDemo)
{
	...
	for (int i = 0; i < numObjects; i++)
	{
		...
		btRigidBodyConstructionInfo rbInfo(mass,myMotionState,buoyantShape,localInertia);
		...
		for (int k=0;k<ARRAY_SIZE_Y;k++)
		{
			for (int i=0;i<ARRAY_SIZE_X;i++)
			{
				for(int j = 0;j<ARRAY_SIZE_Z;j++)
				{
					...
					//using motionstate is recommended, it provides interpolation capabilities, and only synchronizes 'active' objects
					btDefaultMotionState* myMotionState = new btDefaultMotionState(startTransform);
					btRigidBodyConstructionInfo rbInfo(mass,myMotionState,colShape,localInertia);
					btRigidBody* body = new btRigidBody(rbInfo);
					fluidDemo->getHfFluidDynamicsWorld()->addRigidBody(body);
				}
			}
